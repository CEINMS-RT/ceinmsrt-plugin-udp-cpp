...
int main(void)
{
	...
	std::cout << "Number of joint position needed: " << int(ReceiveBuf[0]) << std::endl;

	if (int(ReceiveBuf[0]) != 2)
	{
		std::cout << "ERROR: Number of Channel needed different from 2." << std::endl;
		closesocket(socketS);
		WSACleanup();
		return 0;
	}
