...
int main(void)
{
	...
	while (!stop)
	{
		recvfrom(socketS, ReceiveBuf, sizeof(double) * 2, 0, (sockaddr*)&from, &fromlen);
		memcpy(tempData.data(), ReceiveBuf, sizeof(double) * 2);
		std::cout << tempData[1] << std::endl;
		Sleep(10);
	}
	closesocket(socketS);
	WSACleanup();
